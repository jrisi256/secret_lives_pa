...
data_mod <- data |> filter(county != "blair")
data_nested_county <- data_mod |> nest(.by = county)

################################################################################
# Estimate full model on each county.
################################################################################
data_nested_county <-
    data_nested_county |>
    mutate(
                ...
                function(df) {
                    glmer(
                        bail_decision_bin_nr ~
                            sex + race_collapsed + main_defense_private +
                            age_scaled + I(age_scaled ^ 2) + highest_charge_max +
                            year_cat + (1 | judge_assigned) + (1 | main_defense) +
                            (1 | main_prosecutor) + (1 | judge_defense_dyad) +
            ...
            map(
                ...
                function(df) {
                    glmer(
                        bail_decision_bin_nr ~
                            sex + race_collapsed + main_defense_private +
                            age_scaled + I(age_scaled ^ 2) + highest_charge_min +
                            year_cat + (1 | judge_assigned) + (1 | main_defense) +
                            (1 | main_prosecutor) + (1 | judge_defense_dyad) +
                            (1 | judge_prosecutor_dyad) + (1 | defense_prosecutor_dyad),
                        family = "binomial",
...
saveRDS(data_nested_county, file.path(write_dir, "stratified_models.rds"))
